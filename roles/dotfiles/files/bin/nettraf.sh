...
update_rate() {
  local time=$(date +%s)
  local rx=0 tx=0 total=0 tmp_rx tmp_tx
  ...
  if [ $interval -gt 0 ]; then
    ...
    rate="$(readable $(( (total - last_total) / interval )))"
  else
    rate=""
  fi
...
do
  mem=$(awk '/MemTotal/ {memtotal=$2}; /MemAvailable/ {memavail=$2}; END { printf("%.0f", (100- (memavail / memtotal * 100))) }' /proc/meminfo)
  read line
  update_rate
  echo ",[{\"full_text\":\"⇌ ${rate}\" }, {\"full_text\":\" Σ ${mem}% \"} ,${line#,\[}" || exit 1
done)
